...
def test_complete_workflow():
    ...
    # Step 6: Test retrieval (with corrected API)
    ...
    try:
        ...
        if result['success']:
            ...
            print(f"  Reprojection error: {result.get('reprojection_error', 'N/A'):.2f} px" if isinstance(result.get('reprojection_error'), (int, float)) else f"  Reprojection error: {result.get('reprojection_error', 'N/A')}")
            ...
            print(f"✗ Relocalization failed: {result.get('reason', 'Unknown reason')}")
    except Exception as e:
        print(f"⚠ Pipeline test skipped: {type(e).__name__}: {e}")
    ...
    print(f"  2. See MAP_BUILDING_TUTORIAL.md for detailed guide")
    print(f"  3. Integrate with ROS 2 (Phase 3)")
